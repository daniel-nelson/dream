...
function readonlyArrayTypeNode(elementType: ts.TypeNode): ts.TypeNode {
  return f.createTypeOperatorNode(ts.SyntaxKind.ReadonlyKeyword, f.createArrayTypeNode(elementType))
}
                            ...
                            f.createPropertyAssignment(
                              ...
                              enumTypeNode
                                ? f.createAsExpression(
                                    f.createArrayLiteralExpression(),
                                    readonlyArrayTypeNode(enumTypeNode)
                                  )
                                : f.createNull()
                            ),
    ...
    const types = compact(
      extractedPropertyTypes.map(typeNode => {
        ...
        defaultReturnNode = /^ArrayType/.test(modifiedText)
          ? readonlyArrayTypeNode(this.getFirstGenericType(defaultReturnNode)!)
          : defaultReturnNode
        ...
        switch (sanitized) {
          ...
          case 'Numeric[]':
            return readonlyArrayTypeNode(f.createKeywordTypeNode(ts.SyntaxKind.NumberKeyword))
          ...
          case 'Timestamp[]':
            return isDate
              ? readonlyArrayTypeNode(f.createTypeReferenceNode('CalendarDate'))
              : readonlyArrayTypeNode(f.createTypeReferenceNode('DateTime'))
          ...
          case 'string[]':
            // Kysely-codegen types TIME[] columns as string[], we need to check dbType
            if (isTime) return readonlyArrayTypeNode(f.createTypeReferenceNode('ClockTime'))
            if (isTimeTz) return readonlyArrayTypeNode(f.createTypeReferenceNode('ClockTimeTz'))
            return defaultReturnNode
          ...
          case 'Int8[]':
            return readonlyArrayTypeNode(f.createKeywordTypeNode(ts.SyntaxKind.StringKeyword))
